...
execute_sincronize_natural_evol_alignments=True
...
if(execute_natural_msa):
    auc.msa_set_reference(data_path+"natural/PF00085.fasta")
    auc.buslje09(data_path+"natural/PF00085_THIO_ECOLI_reference.fasta",zmip_natural_result_path)        
for s in structures:
    pdb_name=s
    ...
    if(execute_table_results):
        table.generate_table(data_path+'results/auc.dat', data_path+'results/auc_table.dat', beta, run, nsus)
    if(execute_plot):
        plot.plot_auc(data_path+'results/auc.dat',data_path+'results', beta, run, nsus)
    if(execute_natural_auc):
        utils.sincronize_contact_map(contact_map,contact_map+"sync",2,106)
        auc.auc(data_path+"natural/PF00085_THIO_ECOLI_reference.fasta", contact_map+"sync")
    if(execute_spearman):
