...
printf("initial = %d \n",initial);
 //if((contactmap=fopen("Contact_Map.dat","w"))==NULL) {
if((contactmap=fopen(Options.print_sequences_contact_map_outputfile,"w"))==NULL) {
             ...
             //if(atom[j].sequential != thisresidue) {
            	//if(1==1) {
            	   ...
            	   //if(1==1) {
                       ...
                       //if((sqrt(dist) - atom[i].vw - atom[j].vw) < 2.00){
                       //if(sqrt(dist) < 6.05){
					  ...
					  //fprintf(contactmap,"%d %d %c %d %d %s %s %f %f %f %f \n ",atom[i].sequential,atom[j].sequential,atom[j].chain[0],i+1,j+1,atom[i].atomgroup,atom[j].atomgroup, sqrt(dist),atom[i].vw,atom[j].vw,(sqrt(dist)-atom[i].vw-atom[j].vw));
...
/*                     printf("%d %d %c %d %d %s %s %f %f %f %f \n",atom[i].sequential,atom[j].sequential,atom[j].chain[0],i+1,j+1,atom[i].atomgroup,atom[j].atomgroup, sqrt(dist),atom[i].vw,atom[j].vw,(sqrt(dist)-atom[i].vw-atom[j].vw));*/
  ...
  //fprintf(contactmap," %d %d", protsize,protsize);
  for(i=0;i<protsize;++i) {
            for(j=0;j<total_length;++j) {
				if(j<protsize){
...
int set_eijmat(float eijmat[])
{
  ...
  if((fp=fopen("scpe/scpe_program/ematrix.dat","r"))==NULL) {
